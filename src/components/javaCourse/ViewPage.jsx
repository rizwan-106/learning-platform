...
const ViewPage = () => {
    ...
    const loadContent = async () => {
      try {
        const content = await import(
          `./${name.charAt(0).toUpperCase() + name.slice(1)}Contents.jsx`
        );
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-[calc(100vh-4rem)] border border-gray-200 shadow-lg rounded-lg bg-white font-sans overflow-hidden">
        ...
        <ul className="space-y-1">
          ...
          {courseData?.map((ele, ind) => (
            <li
              key={ele.key || ind}
              className={`
                px-2 py-1 cursor-pointer text-lg rounded-md transition-all duration-200 ease-in-out
                ${
                  ind === selectedIndex
